...
export default async function AnimePage({params} : {params: {id: string}}) {
    const jikanSearch = await axios.get(`https://api.jikan.moe/v4/anime?q=${params.id}`)
    ...
    if(search.totalResults == 0) 
        return (<div>An unexpected error occured and the developer is not paid enough for this.</div>)
    const saturnAnimeInfo = await animesaturn.fetchAnimeInfo(search.results[0].id)
    return (
        <main className="min-h-screen mt-28">
            <div className="block md:flex h-fit mx-8"> 
                ...
                />
                <div>
                    <h1 className="mr-8 z-10 text-2xl font-bold text-pretty">{animeData?.title as string}</h1>
                ...
                <div className="grid gap-3 grid-cols-1 md:grid-cols-2">
                    {
                        saturnAnimeInfo?.episodes?.map((episode, index) => (
                            <Link className="bg-gray-900 rounded-md p-3" key={index} href={`/watch/${episode.id}`}>
